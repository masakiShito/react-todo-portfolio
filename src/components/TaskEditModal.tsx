...
import { fromDateTimeLocal, normalizeDateRange, toDateInput, toDateTimeLocal } from '../utils/dateTime';
...
const TaskEditModal = ({ task, tasks, onSave, onClose }: TaskEditModalProps) => {
  ...
  useEffect(() => {
    ...
    setStartDate(task.startDate ?? toDateInput(task.dueDate));
    setEndDate(task.endDate ?? toDateInput(task.dueDate));
